...
// Global data
// ----------------------------------------------------------------------------
typedef struct {        // WiFi access point data
    char ssid[32];
    char rssi;
    char enc;
} ApData;

typedef struct {         // Scan result
    char scanInProgress; // if 1, don't access the underlying stuff from the webpage.
    ApData **apData;
    int noAps;
} ScanResultData;

ScanResultData cgiWifiAps;    // Static scan status storage.
...
void ICACHE_FLASH_ATTR wifiScanDoneCb(void *arg, STATUS status)
{
    ...
    if(cgiWifiAps.apData != NULL) {     // Clear prev ap data if needed.
    ...
    n = 0;        // Copy access point data to the static struct
    ...
    cgiWifiAps.scanInProgress = 0;   // done.
}
...
int ICACHE_FLASH_ATTR cgiWiFiScan(HttpdConnData *connData)
{
    ...
    if(cgiWifiAps.scanInProgress == 1) {    // We're still scanning. Tell Javascript code that.
        len=os_sprintf(buff, "{\n \"result\": { \n\"inProgress\": \"1\"\n }\n}\n");
        httpdSend(connData, buff, len);
    }
    else {     // We have a scan result. Pass it on.
        len = os_sprintf(buff, "{\n \"result\": { \n\"inProgress\": \"0\", \n\"APs\": [\n");
        httpdSend(connData, buff, len);
        if(cgiWifiAps.apData==NULL) cgiWifiAps.noAps=0;
        for(i = 0; i < cgiWifiAps.noAps; i++) {   // Fill in json code for an access point
        ...
        len = os_sprintf(buff, "]\n}\n}\n");
        httpdSend(connData, buff, len);
        wifiStartScan();        // Also start a new scan.
    }
...
static void ICACHE_FLASH_ATTR resetTimerCb(void *arg)
{
    ...
    if(x == STATION_GOT_IP) {      // Go to STA mode. This needs a reset, so do that.
        os_printf("Obtained IP, Going into STA mode..\n");
        wifi_set_opmode(1);
        system_restart();
    }
    else {         // Maybe also pass this through on the webpage?
...
static void ICACHE_FLASH_ATTR reassTimerCb(void *arg)
{
    ...
    x = wifi_get_opmode();
    if(x != 1) {                            // Schedule disconnect/connect
...
int ICACHE_FLASH_ATTR cgiWiFiConnect(HttpdConnData *connData)
{
    ...
    os_timer_disarm(&reassTimer);    // Schedule disconnect/connect
