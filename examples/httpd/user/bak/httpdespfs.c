...
// ----------------------------------------------------------------------------
// This is a catch-all cgi function. It takes the url passed to it, looks up the
// corresponding path in the filesystem and if it exists, passes the file through.
// This simulates what a normal webserver would do with static files.
// ----------------------------------------------------------------------------
...
int ICACHE_FLASH_ATTR cgiEspFsTemplate(HttpdConnData *connData)
{
    ...
    if(tpd == NULL) {   // First call to this cgi. Open the file so we can read it.
    ...
    if(len > 0) {
        ...
        e = buff;
        for(x = 0; x < len; x++) {
            if(tpd->tokenPos == -1) {     // Inside ordinary text.
                if(buff[x] == '%') {      // Send raw data up to now
                    if(sp != 0) httpdSend(connData, e, sp);
                    sp = 0;
                    tpd->tokenPos = 0;  // Go collect token chars.
                }
                else {
                    sp++;
                }
    ...
    if(sp != 0) httpdSend(connData, e, sp);       // Send remaining bit.
    if(len != 1024) {                             // We're done.
        ((TplCallback)(connData->cgiArg))(connData, NULL, &tpd->tplArg);
        espFsClose(tpd->file);
        return HTTPD_CGI_DONE;
    }
