...
// abstracted operations on the files that are in there.
...
// This simplifies debugging, but needs some slightly different headers.
// The #ifdef takes care of that.
// ----------------------------------------------------------------------------
#ifdef __ets__                   // esp build
#include "c_types.h"
...
// This is using 0.9.1/0.9.2 SDK on a not-too-new module.
...
EspFsFile ICACHE_FLASH_ATTR *espFsOpen(char *fileName)
{
    ...
    EspFsHeader h;
    EspFsFile *r;
    while(fileName[0] == '/') fileName++;     // Strip initial slashes
    while(1) {                              // Go find that file!
        hpos = p;
        os_memcpy(&h, p, sizeof(EspFsHeader));     // Grab the next file header.
...
//          os_printf("Alloc %p\n", r);
            ...
            else if(h.compression == COMPRESS_HEATSHRINK) {  // File is compressed with Heatshrink.
                char parm;
                heatshrink_decoder *dec;                    // Decoder params are stored in 1st byte.
...
int ICACHE_FLASH_ATTR espFsRead(EspFsFile *fh, char *buff, int len)
{
    ...
    if(fh->decompressor==COMPRESS_NONE) { // Do stuff depending on the way the file is compressed.
        int toRead;
