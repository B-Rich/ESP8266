...
void hspi_Tx(uint8_t * data, uint8_t numberByte, uint32_t numberRepeat)
{
    ...
    if((numberByte < 1) || (numberRepeat < 1)) return;    // Error parameter
    ...
    if(numberRepeat > 1)  {
        ...
        if(numberByte > MAX_SIZE_BUFFER) return; // Require describe this case or exit :)

        if(amountDataAtPackage > numberRepeat) amountDataAtPackage = numberRepeat;
