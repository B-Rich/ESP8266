...
static void ICACHE_FLASH_ATTR receive_callback(void * arg, char * buf, unsigned short len)
{
    ...
    if(new_size > BUFFER_SIZE_MAX || NULL == (new_buffer = (char *)os_malloc(new_size))) {
        ...
        os_free(req->buffer);
        req->buffer = NULL;
        return;        // TODO: espconn_disconnect(conn) without crashing.
    }
...
static void ICACHE_FLASH_ATTR sent_callback(void * arg)
{
    ...
    else {           // The headers were sent, now send the contents.
...
void ICACHE_FLASH_ATTR http_raw_request(const char * hostname, int port, const char * path, const char * post_data, http_callback user_callback)
{
    ...
    ip_addr_t addr;  // It seems we don't need a real espconn pointer here.
    err_t error = espconn_gethostbyname((struct espconn *)req, hostname, &addr, dns_callback);
    if(error == ESPCONN_INPROGRESS) {
        HTTP_DEBUG("DNS pending\n");
    }
    else if(error == ESPCONN_OK) {   // Already in the local names table (or hostname was an IP address), execute the callback ourselves.
