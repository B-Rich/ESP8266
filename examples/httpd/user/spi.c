...
void spi_init_gpio(uint8 spi_no, uint8 sysclk_as_spiclk)
{
    ...
    if(sysclk_as_spiclk) {
        ...
        #if DEBUG_SPI_MASTER
            con_printf("Use SPI clkdiv\n");
        #endif
    }
    if(spi_no == SPI) {
        WRITE_PERI_REG(PERIPHS_IO_MUX, 0x0005|(clock_div_flag<<8)); // Set bit 8 if 80MHz sysclock required
        ...
        #if DEBUG_SPI_MASTER
            con_printf("HSPI Config\n");
        #endif
        WRITE_PERI_REG(PERIPHS_IO_MUX, 0x0105|(clock_div_flag << 9)); // Set bit 9 if 80MHz sysclock required
...
uint32 spi_transaction(uint8 spi_no, uint8 cmd_bits, uint16 cmd_data, uint32 addr_bits, uint32 addr_data, uint32 dout_bits, uint32 dout_data, uint32 din_bits, uint32 dummy_bits)
{
    ...
    if(cmd_bits) {
        ...
        #if DEBUG_SPI_MASTER
            con_printf("HSPI command transmitted\n");
        #endif
    }
    // ------------------------------------------------------------------------
    ...
    if(addr_bits) {
        ...
        #if DEBUG_SPI_MASTER
            con_printf("HSPI address transmitted\n");
        #endif
    }
    // ------------------------------------------------------------------------
    ...
    if(dout_bits) {
        ...
        if(READ_PERI_REG(SPI_USER(spi_no))&SPI_WR_BYTE_ORDER) {
            WRITE_PERI_REG(SPI_W0(spi_no), dout_data<<(32-dout_bits));  // copy data to W0
        }
        else {
            ...
            if(dout_extra_bits) {
                ...
                // for example, 0xDA4 12 bits without SPI_WR_BYTE_ORDER would usually be output as
                // if it were 0x0DA4, of which 0xA4, and then 0x0 would be shifted out
                // (first 8 bits of low byte, then 4 MSB bits of high byte - ie reverse byte order).
        ...
        #if DEBUG_SPI_MASTER
             con_printf("HSPI data transmitted\n");
        #endif
    }
    // ------------------------------------------------------------------------
    ...
    if(din_bits) {
        ...
        #if DEBUG_SPI_MASTER
             con_printf("HSPI receving data\n");
        #endif
        if(READ_PERI_REG(SPI_USER(spi_no)) & SPI_RD_BYTE_ORDER) {
            #if DEBUG_SPI_MASTER
                uint8 data = READ_PERI_REG(SPI_W0(spi_no)) >> (32-din_bits);
                con_printf("receive data1 = %x\n", data);
            #endif
            return READ_PERI_REG(SPI_W0(spi_no)) >> (32-din_bits); // Assuming data in is written to MSB. TBC
        }
        else {
            #if DEBUG_SPI_MASTER
                uint8 data = READ_PERI_REG(SPI_W0(spi_no));
                con_printf("receive data2 = %x\n", data);
            #endif
