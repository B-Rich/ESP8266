...
static void parsepour(char *jsdata,  int len)
{
    ...
    os_printf("pour commands to ensue\n");
}
...
//        int i; for(i=0; i<len; i++) if(response[i] == '{') break;
//        int j; for(j=i; j<len-i; j++) if(response[j] == 0x0d) { response[j] = 0; break; }
        char *json = (char *)os_strstr(response, "{\"pour\":"); // find first occurence of...
        char *eol = os_strchr(response, 0x0d);             // Pointer to end of json.
        *eol = 0; // terminate it there
