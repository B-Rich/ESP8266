...
static void tcpclient_discon_cb(void *arg)
{
    ...
    #ifdef DEBUG_MSGS
        con_printf("tcpclient_discon_cb\r\n");
    #endif
    if(pespconn == NULL){
        #ifdef DEBUG_MSGS
            con_printf("tcpclient_discon_cb - conn is NULL!\r\n");
        #endif
        return;
    }
    #ifdef PLATFORM_DEBUG
        con_printf("Will reconnect in 2s...\r\n");
    #endif
...
static void tcpclient_recon_cb(void *arg, sint8 err)
{
    ...
    if(++tcpReconCount >= 5)  {
        ...
        tcpReconCount = 0;
        #ifdef DEBUG_MSGS
            con_printf("tcpclient_recon_cb, 5 failed TCP attempts!\r\n");
            con_printf("Will reconnect in 10s...\r\n");
        #endif
        os_timer_disarm(&WiFiLinker);
        os_timer_setfn(&WiFiLinker, (os_timer_func_t *)platform_reconnect, pespconn);
...
static void tcpclient_connect_cb(void *arg)
{
    ...
    #ifdef DEBUG_MSGS
        con_printf("tcpclient_connect_cb\r\n");
    #endif
    ...
    if(espsent_status == ESPCONN_OK) {
        ...
        #ifdef DEBUG_MSGS
            con_printf("Data sent, payload: %s\r\n", payload);
        #endif
    }
    else {
        connState = TCP_SENDING_DATA_ERROR;
        #ifdef DEBUG_MSGS
            con_printf("Error while sending data.\r\n");
        #endif
    }
}
...
static void senddata(void)
{
    ...
    #ifdef DEBUG_MSGS
        con_printf("Start espconn_connect to " IPSTR ":%d\r\n", IP2STR(Conn.proto.tcp->remote_ip), Conn.proto.tcp->remote_port);
    #endif
    sint8 espcon_status = espconn_connect(&Conn);
    #ifdef DEBUG_MSGS
        switch(espcon_status) {
            case ESPCONN_OK:      con_printf("TCP created.\r\n");                        break;
            case ESPCONN_RTE:     con_printf("Error connection, routing problem.\r\n");  break;
            case ESPCONN_TIMEOUT: con_printf("Error connection, timeout.\r\n");          break;
            default:              con_printf("Connection error: %d\r\n", espcon_status); break;
        }
    #endif
    ...
    switch(wifi_station_get_connect_status()) {
        case STATION_GOT_IP:
            ...
            if(ipConfig.ip.addr != 0) {
                ...
                #ifdef DEBUG_MSGS
                    con_printf("WiFi connected\r\n");
                    con_printf("Start TCP connecting...\r\n");
                #endif
        ...
        case STATION_WRONG_PASSWORD:
            ...
            #ifdef DEBUG_MSGS
                con_printf("WiFi connecting error, wrong password\r\n");
            #endif
            break;
        case STATION_NO_AP_FOUND:
            connState = WIFI_CONNECTING_ERROR;
            #ifdef DEBUG_MSGS
                 con_printf("WiFi connecting error, ap not found\r\n");
            #endif
            break;
        case STATION_CONNECT_FAIL:
            connState = WIFI_CONNECTING_ERROR;
            #ifdef DEBUG_MSGS
                 con_printf("WiFi connecting fail\r\n");
            #endif
            break;
        default:
            connState = WIFI_CONNECTING;
            #ifdef DEBUG_MSGS
                 con_printf("WiFi connecting...\r\n");
            #endif
            break;
    }
    os_timer_setfn(&WiFiLinker, (os_timer_func_t *)wifi_check_ip, NULL);
...
//  wifi_station_set_auto_connect(1);
    #ifdef DEBUG_MSGS
        con_printf("ESP8266 in STA mode configured.\r\n");
    #endif
}
...
void disconnect(void)
{
    ...
    #ifdef DEBUG_MSGS
        con_printf("ESP8266 disconnected.\r\n");
    #endif
}
...
{
    con_printf("\n\ryifi commander:\n\r");
    con_printf("  c - Wifi Connect\n\r");
    con_printf("  d - Wifi Disconnect\n\r");
    con_printf("  i - TCP Connect\n\r");
    con_printf("L/l -LED on/off\n\r");
}
