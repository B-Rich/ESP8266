...
#define SCANLED          5   // Use GPIO 5 for the Scan Indication
...
// gpio_output_set is overkill. ToDo: use better mactos
// ----------------------------------------------------------------------------
void ICACHE_FLASH_ATTR scanLED(int ena)
{
    if(ena) GPIO_OUTPUT_SET(SCANLED, 1);
    else    GPIO_OUTPUT_SET(SCANLED, 0);
}
...
void ioInit(void)
{
    ...
    gpio_output_set(0, 0, (1 << SCANLED), 0);                // Set BUSYLED GPIO to output
